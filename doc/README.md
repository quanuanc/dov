...
DOV 是一个基于“画面像素”的文件传输工具：Hermes（发送端）把文件编码为视频帧输出到屏幕/HDMI，Argus（接收端）通过采集卡逐帧解码并还原文件。
...
- `hermes/`：Hermes（发送端，JavaFX 全屏）
- `argus/`：Argus（接收端，JavaCV + Swing）
- `doc/`：设计与逻辑文档
...
```bash
mvn -pl hermes -am package
mvn -pl argus -am package
```

## 运行

```bash
java -jar hermes/target/hermes-1.0-shaded.jar
java -jar argus/target/argus-1.0-shaded.jar
```
...
```bash
java -Ddov.config=/path/to/dov.properties -jar hermes/target/hermes-1.0-shaded.jar
```
...
注意：Hermes 与 Argus 的配置必须一致，否则解码会失败或误码率升高。
